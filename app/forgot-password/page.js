  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Show reset link for testing in development
        if (data.resetLink) {
          setResetInfo({
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
        ...
        {resetInfo && (
          <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg mb-4">
            <h3 className="font-semibold text-yellow-800 mb-2">Reset Link (Development):</h3>
            <p className="text-sm text-yellow-700 break-all mb-2">
              {resetInfo.resetLink}
            </p>
            <a 
